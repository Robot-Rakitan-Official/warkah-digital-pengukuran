...
$where = $search ? "WHERE nama_lengkap LIKE '%$search%' OR nip LIKE '%$search%' OR jabatan LIKE '%$search%'" : "";

$batas_data = 5; 
$halaman_aktif = isset($_GET['halaman']) ? (int)$_GET['halaman'] : 1;
$offset = ($halaman_aktif - 1) * $batas_data;

$query_count = "SELECT COUNT(*) AS total FROM anggota $where";
$result_count = mysqli_query($conn, $query_count);
$row_count = mysqli_fetch_assoc($result_count);
$total_data = $row_count['total'];
$total_halaman = ceil($total_data / $batas_data);

$query = "SELECT * FROM anggota $where ORDER BY id DESC LIMIT $batas_data OFFSET $offset";
...
<head>
    ...
    <title>Data Anggota - Warkah Digital Pengukuran</title>
...
<body class="flex flex-col md:flex-row h-screen overflow-hidden text-gray-800">
                ...
                <div>
                    ...
                    <div id="master-submenu" class="pl-12 py-1 space-y-1">
                        <?php if ($user_role == 'admin'): ?>
                            <a href="data_anggota.php" class="block py-2 text-sm transition-colors text-white font-bold border-l-2 border-white/70 pl-2">
                                Data Anggota
                            </a>
                        <?php endif; ?>
                        <a href="data_gambar_ukur.php" class="block py-2 text-sm transition-colors text-gray-400 hover:text-white font-semibold">
                            Data Gambar Ukur
                        </a>
                    </div>
                </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden w-full relative z-0">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-10 bg-gray-50/30 w-full">
            ...
            <p class="text-xs md:text-sm text-gray-500 mb-6 font-medium">Data Anggota Pengukur</p>
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm w-full">
                <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center p-4 md:p-6 border-b border-gray-50 gap-4">
                    <form method="GET" class="relative w-full sm:w-80">
                        ...
                        <input type="text" name="cari" value="<?= htmlspecialchars($search); ?>" placeholder="Cari data anggota..." class="w-full pl-10 p-2.5 border border-gray-200 rounded-xl text-xs md:text-sm focus:outline-none focus:border-navy transition-all">
                    </form>
                    <a href="tambah_anggota.php" class="bg-navy text-white px-4 md:px-5 py-2.5 rounded-xl text-xs md:text-sm font-bold hover:bg-opacity-90 transition-all shadow-md flex items-center justify-center gap-2">
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="bg-gray-50/50 text-gray-500 text-[10px] uppercase font-bold tracking-widest border-b border-gray-100">
                            <tr>
                                ...
                                <th class="py-4 px-4 md:px-6">NIP</th>
                                <th class="py-4 px-4 md:px-6">NAMA LENGKAP</th>
                                <th class="py-4 px-4 md:px-6">JABATAN</th>
                                <th class="py-4 px-4 md:px-6">EMAIL</th>
                                <th class="py-4 px-4 md:px-6">NO. TELEPON</th>
                        ...
                        <tbody class="text-gray-600 text-[11px] md:text-xs font-medium">
                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4 md:px-6 font-bold text-gray-700"><?= htmlspecialchars($row['nip']); ?></td>
                                <td class="py-4 px-4 md:px-6 font-bold text-navy"><?= htmlspecialchars($row['nama_lengkap']); ?></td>
                                <td class="py-4 px-4 md:px-6"><?= htmlspecialchars($row['jabatan']); ?></td>
                                <td class="py-4 px-4 md:px-6 text-gray-500"><?= htmlspecialchars($row['email']); ?></td>
                                <td class="py-4 px-4 md:px-6 text-gray-500"><?= htmlspecialchars($row['no_telepon']); ?></td>
                                <td class="py-4 px-4 md:px-6 text-center">
                                    <?php if($row['status'] == 'Aktif'): ?>
                                        <span class="bg-green-50 text-green-600 border border-green-100 py-1.5 px-3 rounded-md text-[9px] font-bold tracking-widest uppercase">Aktif</span>
                                    <?php else: ?>
                                        <span class="bg-red-50 text-red-600 border border-red-100 py-1.5 px-3 rounded-md text-[9px] font-bold tracking-widest uppercase">Tidak Aktif</span>
                                    <?php endif; ?>
                                </td>
                                <td class="py-4 px-4 md:px-6 text-center flex justify-center gap-2 md:gap-3">
                                    <a href="edit_anggota.php?id=<?= $row['id']; ?>" class="text-gray-400 hover:text-navy transition-colors bg-white border border-gray-100 p-1.5 rounded-lg shadow-sm" title="Edit Anggota">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <a href="#" onclick="bukaModalHapus(event, 'hapus_anggota.php?id=<?= $row['id']; ?>')" class="text-gray-400 hover:text-red-600 transition-colors bg-white border border-gray-100 p-1.5 rounded-lg shadow-sm" title="Hapus Anggota">
                            ...
                            <tr>
                                <td colspan="8" class="py-12 text-center text-gray-400 font-bold italic">Data anggota tidak ditemukan.</td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    <div id="modalHapus" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm transition-all no-print">
        <div class="bg-white rounded-[1.5rem] p-6 max-w-sm w-full mx-4 shadow-2xl border border-gray-100 animate-fade-in text-center relative overflow-hidden">
            ...
            <h3 class="font-extrabold text-[#110B45] text-lg mb-2 font-['Public_Sans']">Hapus Anggota?</h3>
            <p class="text-[11px] text-gray-500 font-medium mb-6 leading-relaxed">Anda yakin ingin menghapus data anggota ini? Tindakan ini tidak dapat dibatalkan.</p>
